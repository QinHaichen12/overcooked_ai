...
def _build_agent(agent_kind, mlam):
    ...
    raise ValueError(
        "Unknown agent kind '{}'. Supported: role_aware, state_aware, greedy, random".format(
            agent_kind
        )
    )
...
def main():
    parser = argparse.ArgumentParser(
        description="Compare one or more Overcooked agents against a fixed partner"
    )
    parser.add_argument(
        "--agents",
        nargs="+",
        default=["greedy", "role_aware"],
        help="Agent names to evaluate. Supported: role_aware state_aware greedy random",
    )
    ...
    parser.add_argument(
        ...
        default="greedy",
        help="Teammate policy used for all evaluated agents",
    )
    parser.add_argument(
        "--out",
        default="results/agent_comparison.json",
    ...
    for agent_name in args.agents:
        output["agents"][agent_name] = _evaluate_agent_family(
            layouts=args.layouts,
            episodes=args.episodes,
            horizon=args.horizon,
            focal_kind=agent_name,
    ...
    print("Saved agent comparison summary to {}".format(out_path))
